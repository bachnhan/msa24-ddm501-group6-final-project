...
def train_and_register():
    ...
    print("📂 Loading TELCO MASTER DATA (V6.5 - RAI Integrated)...")
    try:
        path = "telco-customer-churn/WA_Fn-UseC_-Telco-Customer-Churn.csv"
        if not os.path.exists(path):
             print("⚠️ Telco CSV not found locally. Please ensure data is in place.")
             return
    ...
    for m_type in ["xgboost", "random_forest", "logistic_regression"]:
        ...
        with mlflow.start_run(run_name=f"SCRIPT_RAI_{m_type.upper()}"):
            ...
            if gender_col:
                ...
                audit_report_path = f"audit_{m_type}.csv"
