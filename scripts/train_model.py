...
def train_and_register():
    ...
    for m_type in ["xgboost", "random_forest", "logistic_regression"]:
        ...
        with mlflow.start_run(run_name=f"ROUND2_{m_type.upper()}"):
            ...
            pipe = ImbPipeline(
                [("pre", preprocessor), ("smote", SMOTE(random_state=42)), ("clf", clf)]
            )

            # Increased search intensity (n_iter=15, cv=5)
            search = RandomizedSearchCV(
                pipe, m_params, n_iter=15, cv=5, scoring="recall", n_jobs=-1
            )
            ...
            print(
                f"🔍 Finding optimal threshold for {m_type.upper()} (Target: Recall >= 0.80)..."
            )
            ...
            print(
                f"🏆 Round 2 Results ({m_type.upper()}) at Threshold {best_threshold:.4f}:"
            )
            ...
            gender_col = next(
                (c for c in audit_df.columns if c.lower() == "gender"), None
            )
            if gender_col:
