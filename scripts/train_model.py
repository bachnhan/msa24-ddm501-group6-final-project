...
def train_and_register():
    ...
    print("📂 Loading & Profiling Production Data...")
    try:
    ...
    print(f"\n--- 📊 DATA PROFILE ---")
    print(f"Train Size: {len(df_train)} | Val Size: {len(df_val)} | Test Size: {len(df_test_final)}")
    print(f"Churn Recall Goal (Base Rate): {df_train['churn'].mean():.2%}")
    ...
    # Fit preprocessor to transform validation set for XGBoost early stopping
    preprocessor.fit(X_train)
    X_val_transformed = preprocessor.transform(X_val)

    # 4. Ultimate Compact Training
    for m_type in ["xgboost", "random_forest", "logistic_regression"]:
        mlflow.set_experiment("Churn_Master_Full_Audit")
        with mlflow.start_run(run_name=f"SCRIPT_MASTER_{m_type.upper()}"):
            print(f"\n🚀 Training {m_type.upper()} (Compact Mode)...")
            
            if m_type == "xgboost":
                pw = (y_train == 0).sum() / (y_train == 1).sum()
                # Memory safe: max 50 trees
            ...
            print(f"✅ Registered COMPACT: {m_type.upper()}")
